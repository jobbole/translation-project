...
这是很大的声明，这需要一些证明。为了在 2018 年证明这一点，我们必须要小心点。许多的代码使用了异常处理，这的确带来了一些开销。然而，可以说明的是我们的所有代码都是没有异常处理的，这会导致编译器移除那些基础架构。
...
为了确保我们是公平的，因为 `qsort()` 使用了自定义的比较函数，但我们的 `std::sort` 函数没有，因此我们可以使用下面的代码：
...
说起来可能很难相信，但在 C++ 最初开发的大部分时间里，确实没有字符串类。写这么一个类是有种仪式感，并且每个人写都不一样。这背后原因是部分是因为是想尝试写出一个满足所有人的类。
